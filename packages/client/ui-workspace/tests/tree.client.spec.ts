...
import {
  ...
  workspaceLabel, relativeTime, UNGROUPED_KEY,
...
describe('deriveTelegramSessions', () => {
  ...
  it('collects telegram-origin sessions newest first and excludes them elsewhere', () => {
    ...
    const groups = deriveGroups(sessions, [workspace('first', ['local', 'bot-new', 'bot-old'])], noArchive, noAttention, view())
    for (const group of groups) {
      expect(group.sessions.map(node => node.id)).not.toContain(sid('bot-new'))
    }
    expect(deriveFlat(sessions, noArchive, noAttention).map(node => node.id)).toEqual([sid('local')])
  })
