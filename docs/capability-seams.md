...
| `ctx.usage` | `core` | `usage` | - | [`api-remotes`](../packages/api/remotes) | - | Resolves provider credential refs per call, fetches OpenRouter credits and OpenCode Go windows in parallel, and degrades each report independently over the Host unary Remote contract without caching. |
