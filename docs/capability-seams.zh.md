...
flowchart LR
  ...
  pkg_usage["usage"]
  svc_usage["ctx.usage<br/>Connected-provider usage projection"]
...
| `ctx.usage` | `core` | `usage` | - | [`api-remotes`](../packages/api/remotes) | - | 每次调用解析提供商凭据引用，并行请求 OpenRouter 额度与 OpenCode Go 窗口，并在 Host 一元 Remote 契约上让每份报告相互独立地降级，不做缓存。 |
